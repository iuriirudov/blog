...
.get(middleware.redirectLogin, async(req, res) => {
    ...
    const page = {title: 'User Profile'}
    const category = {}
    const categories = await Category.find()
    res.render('user/profile', {page, category, categories, user})
})

router.route('/login')
.get(middleware.redirectHome, async(req, res) => {
    const page = {title: 'Login Form'}
    const category = {}
    const categories = await Category.find()
    const user = res.locals.user
    res.render('user/login', {page, category, categories, user})
})
...
.get(middleware.redirectHome, async(req, res) => {
    const page = {title: 'Registration Form'}
    const category = {}
    const categories = await Category.find()
    const user = res.locals.user
    res.render('user/register', {page, category, categories, user})
})
